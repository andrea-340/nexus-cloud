      ...
      <main className="max-w-7xl mx-auto pt-32 px-6 pb-24">
        ...
        {!activeFolder ? (
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className={`${theme === 'dark' ? 'bg-slate-900/80 backdrop-blur-xl border-slate-700' : 'bg-white border-slate-200'} rounded-[2rem] sm:rounded-[2.75rem] md:rounded-[3.5rem] p-4 sm:p-6 md:p-12 shadow-2xl border relative min-h-[600px]`}>
            ...
            <div className="flex items-center gap-3 sm:gap-4 mb-8 sm:mb-12 pr-12 sm:pr-16">
               <button
                 onClick={() => {
               ...
               <h2 className={`text-lg sm:text-2xl md:text-3xl font-black italic uppercase tracking-tighter break-words ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Archivio / <span className="text-blue-600">{activeFolder}</span> {activeSubFolder && <span className="text-slate-400"> / {activeSubFolder}</span>} {activeClientFolder && <span className="text-slate-400"> / {activeClientFolder}</span>}</h2>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  ...
                  {activeFolder === "Lavoro" && activeSubFolder === "Preventivi" && activeClientFolder && haPIva && (
                    <button
                      ...
                      className="aspect-square bg-gradient-to-br from-blue-50 to-white rounded-[2.5rem] border-2 border-dashed border-blue-200 flex flex-col items-center justify-center hover:border-blue-600 hover:bg-blue-100/40 transition-all group overflow-hidden active:scale-[0.99] relative"
                    >
                  ...
                  {userFiles.map((f, i) => (
                    f.isFolder ? (
                      <motion.button
                        ...
                        className="aspect-square bg-white border border-slate-100 rounded-[2.5rem] shadow-sm flex flex-col items-center justify-center group relative overflow-hidden transition-all hover:shadow-xl"
                      >
                        <Folder size={40} className="text-blue-600 mb-3 group-hover:scale-110 transition-transform" />
                        <p className="text-[10px] font-black uppercase w-full px-6 text-center text-slate-600 break-words leading-relaxed">{f.name}</p>
