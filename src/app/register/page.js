...
export default function Register() {
  ...
  const handleAction = async (e) => {
    ...
    const normalizedBirthDate = dataNascita.trim();

    if (!normalizedEmail || !normalizedPassword || !fullName || !normalizedBirthDate) {
      setErrorMsg("Compila tutti i campi richiesti.");
      setLoading(false);
      return;
    }
    ...
    if (error) {
      ...
      if (data?.user?.id) {
        await supabase.from("profiles").upsert({
          id: data.user.id,
          email: normalizedEmail,
          full_name: fullName || "Operatore Nexus",
          birth_date: normalizedBirthDate,
          piano: "Free Plan",
        });
      }
